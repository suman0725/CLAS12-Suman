...
void multiplicityRatio1_new4_new() {
    ...
    auto* h_LD2_e_P =  new TH1F("h_electron","LD2_trigger_electron_momentum", 100, 1, 10); 
    ...
    while (chainLD.Next()) {
        ...
        for (auto& p : particles) {
            ...
            if (pid == 11) { // Electron
                hasElectron = true;  // Set flag for electron
		    ...
		    elec_mom_LD2 = p->getP(); 
                ...
                hasHadron = true;  // Set flag for hadron
            }
        }

        // Only fill histograms if both an electron and a hadron are present
        if (hasElectron && hasHadron && eventPassesCuts) {
    ...
    graph->Draw("AP");
    canvas->SaveAs("multiplicity_ratio_vs_nu_with_cuts_018451.png");
